...
def format_bullet_points(points: List[str], indent: str = "") -> str:
    """Format a list of points into a hierarchical bullet-point structure."""
    formatted_points = []
    current_section = ""
...
def process_text(text, mode="detailed"):
    try:
        print("Starting text processing...", file=sys.stderr)
        ...
        if nlp is None:
            print("Initializing spaCy...", file=sys.stderr)
            nlp = spacy.load("en_core_web_sm")
        
        if summarizer is None:
            print("Initializing summarizer...", file=sys.stderr)
            summarizer = get_summarizer()
        
        # Process with spaCy
        print("Processing text with spaCy...", file=sys.stderr)
        doc = nlp(text)
        ...
        try:
            ...
            output.append("[SUMMARY]")
            output.append("Error generating summary")
        
        # Extract key concepts (named entities)
        print("Extracting key concepts...", file=sys.stderr)
        key_concepts = []
        for ent in doc.ents:
            concept = f"{ent.text} ({ent.label_})"
            key_concepts.append(concept)
        print(f"Found {len(key_concepts)} key concepts", file=sys.stderr)
        ...
        final_output = "\n".join(output)
        print("Final output generated successfully", file=sys.stderr)
        return final_output
        
    except Exception as e:
        print(f"Error in process_text: {str(e)}", file=sys.stderr)
        return f"[ERROR]\n{str(e)}"

def main():
    if len(sys.argv) != 2:
        print("[ERROR]\nInvalid number of arguments. Expected: text", file=sys.stderr)
        return
